 ...
 * For flashing/building STM32_Programmer_CLI --connect port=swd --download D:/School/Capstone/PicoAPRS-Firmware/build/Debug/tests/integrated_stop2_test/integrated_stop2_test.elf -hardRst -rst --start
 ******************************************************************************
 */
...
void Enter_Stop2_Mode() {
    printf("Setting RTC Wakeup Timer...\n");
    HAL_RTCEx_DeactivateWakeUpTimer(&hrtc);  // Always deactivate first
    ...
    printf("Configuring Timer to %ds...\n", sec);
     // Suspend SysTick to reduce power
    HAL_SuspendTick();
...
void WakeInit() {
    // Restore system clock (must be done first)
    ...
    // Deactivate wakeup timer to allow reconfiguration later
    printf("Restoring system clock...\n");
    printf("Disabling RTC Wakeup Timer...\n");
...
int main(void) {
    ...
    while(1) {
        ...
        if (value_adc > Threshold) {
            ...
            Enter_Stop2_Mode();
            // If bootup has already been ran just only enable sensors
            WakeInit();
        }
        else {
